...
function get_cell_style(styles, cell, opts) {
	...
	if (typeof style_builder != 'undefined') {
		if (/^\d+$/.exec(cell.s)) { return cell.s; }
		if (cell.s && (cell.s == +cell.s)) { return cell.s; }
		var s = cell.s || {};
...
var XmlNode = (function () {
	...
	XmlNode.prototype.children = function() { return this._children; };
	XmlNode.prototype.append = function(node) { this._children.push(node); return this; };
	XmlNode.prototype.prefix = function(prefix) { if(arguments.length===0) return this._prefix; this._prefix = prefix; return this; };
	XmlNode.prototype.attr = function(attr, value) {
		if (value == null && arguments.length === 2) { delete this._attributes[attr]; return this; }
		if (arguments.length === 0) return this._attributes;
		if (typeof attr == 'object' && arguments.length === 1) { for (var key in attr) this._attributes[key] = attr[key]; return this; }
		if (arguments.length === 2 && typeof attr === 'string') this._attributes[attr] = value;
		return this;
	};
	XmlNode.prototype.escapeAttributeValue = function(v) { return '"' + String(v).replace(/"/g, '&quot;') + '"'; };
	XmlNode.prototype.toXml = function(node) {
		node = node || this;
		var xml = node._prefix + '<' + node.tagName;
		if (node._attributes) for (var key in node._attributes) xml += ' ' + key + '=' + this.escapeAttributeValue(node._attributes[key]);
		if (node._children && node._children.length > 0) {
			xml += '>';
			for (var i = 0; i < node._children.length; i++) xml += this.toXml(node._children[i]);
			xml += '</' + node.tagName + '>';
		} else xml += '/>';
		return xml;
	};
...
var StyleBuilder = function (options) {
	...
	var table_fmt_sb = {
		...
		37:"#,##0 ;(#,##0)",38:"#,##0 ;[Red](#,##0)",39:"#,##0.00;(#,##0.00)",40:"#,##0.00;[Red](#,##0.00)",45:"mm:ss",46:"[h]:mm:ss",47:"mmss.0",48:"##0.0E+0",49:"@",56:'"上午/下午 "hh"時"mm"分"ss"秒 "'
	};
	var fmt_table = {}; for (var idx in table_fmt_sb) fmt_table[table_fmt_sb[idx]] = idx;
	var _hashIndex = {};
	return {
		initialize: function(options) {
			...
			this.$styles = XmlNode("styleSheet")
				...
				.append(this.$numFmts).append(this.$fonts).append(this.$fills).append(this.$borders)
				.append(this.$cellStyleXfs.append(this.$xf)).append(this.$cellXfs).append(this.$cellStyles).append(this.$dxfs).append(this.$tableStyles);
		...
		addStyle: function(attributes) {
			...
			if (index == undefined) { index = this._addXf(attributes); _hashIndex[hashKey] = index; }
			return index;
		},
		addStyles: function(styles) { var self=this; return styles.map(function(style){ return self.addStyle(style); }); },
		_addXf: function(attributes) {
			...
			if (numFmtId > 0) $xf.attr("applyNumberFormat", "1");
		...
		_addFont: function(attributes) {
			...
			if (attributes.vertAlign) $font.append(XmlNode("vertAlign").attr("val", attributes.vertAlign));
			if (attributes.color) {
				if (attributes.color.theme) {
					$font.append(XmlNode("color").attr("theme", attributes.color.theme));
					if (attributes.color.tint) $font.append(XmlNode("tint").attr("theme", attributes.color.tint));
				} else if (attributes.color.rgb) {
					$font.append(XmlNode("color").attr("rgb", attributes.color.rgb));
				}
			}
			this.$fonts.append($font);
			var count = this.$fonts.children().length; this.$fonts.attr("count", count);
			return count - 1;
		},
		_addNumFmt: function(numFmt) {
			if (!numFmt) return 0;
			if (typeof numFmt == "string") { var numFmtIdx = fmt_table[numFmt]; if (numFmtIdx >= 0) return numFmtIdx; }
			if (/^[0-9]+$/.exec(numFmt)) return numFmt;
			numFmt = String(numFmt).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&apos;");
			var $numFmt = XmlNode("numFmt").attr("numFmtId", ++customNumFmtId).attr("formatCode", numFmt);
			this.$numFmts.append($numFmt);
			var count = this.$numFmts.children().length; this.$numFmts.attr("count", count);
		...
		_addFill: function(attributes) {
			...
			if (attributes.fgColor) {
				...
				if (attributes.fgColor.rgb) {
					if (attributes.fgColor.rgb.length == 6) attributes.fgColor.rgb = "FF" + attributes.fgColor.rgb;
					$fgColor.attr("rgb", attributes.fgColor.rgb);
					$patternFill.append($fgColor);
				} else if (attributes.fgColor.theme) {
					$fgColor.attr("theme", attributes.fgColor.theme);
					if (attributes.fgColor.tint) $fgColor.attr("tint", attributes.fgColor.tint);
					$patternFill.append($fgColor);
				}
				if (!attributes.bgColor) attributes.bgColor = { indexed: "64" };
			}
			if (attributes.bgColor) $patternFill.append(XmlNode("bgColor").attr(attributes.bgColor));
			this.$fills.append(XmlNode("fill").append($patternFill));
			var count = this.$fills.children().length; this.$fills.attr("count", count);
			return count - 1;
		},
		_getSubBorder: function(direction, spec) {
			...
			if (spec) {
				...
				if (spec.color) {
					...
					if (spec.color.auto) $color.attr("auto", spec.color.auto);
					else if (spec.color.rgb) $color.attr("rgb", spec.color.rgb);
					else if (spec.color.theme || spec.color.tint) { $color.attr("theme", spec.color.theme || "1"); $color.attr("tint", spec.color.tint || "0"); }
		...
		_addBorder: function(attributes) {
			...
			["left","right","top","bottom","diagonal"].forEach(function(direction){ $border.append(self._getSubBorder(direction, attributes[direction])); });
			this.$borders.append($border);
			var count = this.$borders.children().length; this.$borders.attr("count", count);
			return count - 1;
		},
		toXml: function() { return this.$styles.toXml(); }
	}.initialize(options || {});
};
/* xlsx-js-style ^^^ */
